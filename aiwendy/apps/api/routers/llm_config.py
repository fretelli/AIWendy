...
from infrastructure.llm.custom_api_provider import CustomAPIConfig, APIFormat, AuthType
from infrastructure.llm.base import Message, LLMConfig as BaseLLMConfig
...
class LLMProviderConfig(BaseModel):
    ...
    provider_type: str = Field(..., description="Provider type (openai, anthropic, custom, etc.)")
    ...
    api_format: Optional[str] = Field(default="openai", description="API format (openai, anthropic, google, custom)")
    auth_type: Optional[str] = Field(default="bearer", description="Auth type (bearer, api_key, basic, none)")
    ...
    # Extra configuration
    extra_headers: Optional[Dict[str, str]] = Field(None, description="Extra headers to send")
    extra_body_params: Optional[Dict[str, Any]] = Field(None, description="Extra body parameters")
    ...
    def validate_provider_type(cls, v):
        ...
        if v not in valid_providers and v != "custom":
            raise ValueError(f"Invalid provider type. Must be one of: {', '.join(valid_providers)} or 'custom'")
        return v
...
class TestLLMRequest(BaseModel):
    ...
    message: str = Field(default="Hello! Can you introduce yourself?", description="Test message")
...
class QuickTestRequest(BaseModel):
    ...
    base_url: Optional[str] = Field(None, description="Base URL for custom API endpoints")
...
class FetchModelsRequest(BaseModel):
    ...
    provider_type: str = Field(..., description="Provider type (openai, openrouter, custom, etc.)")
    api_key: Optional[str] = Field(None, description="API key (if required)")
    base_url: Optional[str] = Field(None, description="Override base URL for the provider")
    api_format: Optional[str] = Field(default="openai", description="API format for custom providers")
    auth_type: Optional[str] = Field(default="bearer", description="Auth type for custom providers")
    auth_header_name: Optional[str] = Field(None, description="Custom auth header name for custom providers")
    models_endpoint: Optional[str] = Field(None, description="Models endpoint path for custom providers")
    extra_headers: Optional[Dict[str, str]] = Field(None, description="Extra headers to send")
...
async def list_available_providers() -> Dict[str, Any]:
    ...
    for provider in providers:
        ...
        provider_info.append(LLMProviderInfo(
            type=provider,
    ...
    return {
        ...
        "presets": {
            "cloud": [
                "openai", "anthropic", "groq", "together", "anyscale",
                "perplexity", "deepinfra", "openrouter", "moonshot",
                "zhipu", "baichuan", "qwen", "deepseek"
            ],
            "local": [
                "ollama", "vllm", "localai", "xinference"
            ],
            "proxy": [
                "oneapi", "api2d"
            ]
...
async def get_user_configs(
    ...
    configs = current_user.api_keys_encrypted.get("llm_configs", []) if current_user.api_keys_encrypted else []
...
async def get_models_for_user_config(
    ...
    config = next(
        (c for c in current_user.api_keys_encrypted["llm_configs"] if c.get("id") == config_id),
        None
    )
    if not config:
        raise HTTPException(
...
async def create_user_config(
    ...
    config_dict["available_models"] = _clean_models_list(config_dict.get("available_models"))
    ...
    if config_dict.get("provider_type") == "custom":
        config_dict["chat_endpoint"] = _clean_str(config_dict.get("chat_endpoint")) or "/v1/chat/completions"
        config_dict["completions_endpoint"] = _clean_str(config_dict.get("completions_endpoint")) or "/v1/completions"
        config_dict["embeddings_endpoint"] = _clean_str(config_dict.get("embeddings_endpoint")) or "/v1/embeddings"
        config_dict["models_endpoint"] = _clean_str(config_dict.get("models_endpoint")) or "/v1/models"
...
async def update_user_config(
    ...
    if config_dict.get("api_key") and not config_dict["api_key"].startswith("sk-..."):
        ...
        config_dict["api_key"] = current_user.api_keys_encrypted["llm_configs"][config_index].get("api_key")
...
async def delete_user_config(
    ...
    current_user.api_keys_encrypted["llm_configs"] = [
        c for c in current_user.api_keys_encrypted["llm_configs"]
        if c["id"] != config_id
    ]
    ...
    try:
        if config["provider_type"] == "custom":
            ...
            custom_config = CustomAPIConfig(
                ...
                completions_endpoint=config.get("completions_endpoint", "/v1/completions"),
        ...
        response = await asyncio.wait_for(
            provider.chat(messages, llm_config),
            timeout=30.0
        )
        ...
        if isinstance(unwrapped, AppException):
            raise HTTPException(
                status_code=unwrapped.status_code,
