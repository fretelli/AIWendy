...
def record_task_owner(task_id: str, user_id: str, ttl_seconds: int = 24 * 60 * 60) -> None:
...
def publish_task_event(task_id: str, payload: dict[str, Any]) -> None:
    ...
    redis_client = get_redis_client()
    redis_client.publish(task_event_channel(task_id), json.dumps(payload, ensure_ascii=False))
