...
def upgrade() -> None:
    # Create enum types
    op.execute("CREATE TYPE subscriptiontier AS ENUM ('free', 'pro', 'elite', 'enterprise')")
    op.execute("CREATE TYPE moodtype AS ENUM ('very_negative', 'negative', 'neutral', 'positive', 'very_positive')")
    ...
    op.create_table(
        ...
        sa.Column('subscription_tier', postgresql.ENUM('free', 'pro', 'elite', 'enterprise', name='subscriptiontier'), default='free', nullable=False),
        sa.Column('subscription_expires_at', sa.DateTime(timezone=True), nullable=True),
        sa.Column('preferences', postgresql.JSONB, default=dict, nullable=False),
        sa.Column('notification_settings', postgresql.JSONB, default=dict, nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('mood', postgresql.ENUM('very_negative', 'negative', 'neutral', 'positive', 'very_positive', name='moodtype'), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('dominant_emotions', postgresql.ARRAY(sa.String), default=list, nullable=False),
    ...
    op.create_index('ix_mood_analyses_period', 'mood_analyses', ['period_start', 'period_end'])

    # Create journal_analyses table
    op.create_table(
        'journal_analyses',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column('journal_entry_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('journal_entries.id', ondelete='CASCADE'), nullable=False, unique=True),
    ...
    op.create_index('ix_journal_analyses_journal_entry_id', 'journal_analyses', ['journal_entry_id'])
