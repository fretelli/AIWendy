...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('plan_type', sa.Enum('free', 'pro', 'elite', 'enterprise',
                                       name='plantype'), nullable=False, unique=True),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('active', 'trialing', 'past_due', 'canceled',
                                    'unpaid', 'incomplete', 'incomplete_expired',
                                    name='subscriptionstatus'), nullable=False),
        sa.Column('billing_interval', sa.Enum('monthly', 'yearly', 'lifetime',
                                              name='billinginterval'), nullable=False),
    ...
    op.create_index('ix_user_subscriptions_user_status', 'user_subscriptions',
                    ['user_id', 'status'])
    op.create_index('ix_user_subscriptions_stripe_id', 'user_subscriptions',
    ...
    op.create_table(
        ...
        sa.Column('subscription_id', postgresql.UUID(as_uuid=True),
                  sa.ForeignKey('user_subscriptions.id'), nullable=True),

        # Payment details
        sa.Column('amount', sa.Numeric(10, 2), nullable=False),
        sa.Column('currency', sa.String(3), default='USD'),
        sa.Column('status', sa.Enum('pending', 'processing', 'succeeded', 'failed',
                                    'canceled', 'refunded', name='paymentstatus'),
