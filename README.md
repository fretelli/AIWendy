...
- Market is organized as exactly two top-level views—Market/Sectors/Flows and Macro Data—with rates, futures, options, valuation, correlation, and factors kept as in-module drilldowns. Capability-manifest entries may retire a physical dataset without deleting it; affected endpoints remain HTTP-compatible, return `available=false`, and do not query retained tables. Rates exposes provider-native SHIBOR/LPR, historical USD LIBOR/HIBOR and Wenzhou/Guangzhou private-lending indices, US nominal/real curves, Treasury bills, long-term composite/20Y CMT, and the real long-term average; stopped sources are labeled rather than synthetically extended.
- Interactive valuation v2, 20–252 day correlation, and factor/crowding endpoints read immutable Tushare-published snapshots; each market or macro history range is scoped to its visible drilldown instead of a module-wide hidden control, while valuation bubbles and ranking rows open a 1M–5Y point-in-time view with PE/PB, percentiles, crowding coverage, auditable constituents, and an active-account held-industry filter without request-time source scans. Expected broad-index coverage is explicit instead of being inferred from history length. Macro v4 puts provider-native structure snapshots directly on the six complete overview cards, preserves the rates/yields group, and labels stale histories without hiding them. Structured Tushare datasets take priority; whitelisted `eco_cal` releases require coverage, revision, and freshness disclosure, and missing domains remain explicitly unavailable without synthetic substitutes.
- Macro navigation returns lightweight official-field summaries, featured latest-period fields, and the next scheduled China release or awaiting-source state. GDP/CPI/PPI/M2/social-financing/PMI drilldowns switch between one focused analysis chart and a searchable grouped official-field catalog; clicking a card structure value opens that exact field history. Historical position uses a separately labelled point-in-time 5Y, 10Y, 20Y, or expanding benchmark while the display range controls only the chart axis. PPI keeps every synchronized production-material and consumer-goods subseries across YoY, MoM, and accumulated YoY measures.
