...
- 机会中心：从市场二级导航提升为 Research OS 一级入口，汇合全市场变化、我的公司和股东证据；旧地址永久重定向，机会快照可带入研究或建立人工 TAA 草案，但不会自动生成权重或下单。
- 家庭财富配置框架：支持单人和多成员家庭，记录年龄、退休阶段、收入、资产负债与目标，形成安全/市场/进取三层、短中长期资金桶和核心-卫星边界；框架、SAA 与 TAA 都保存不可变版本。TAA 只能建立在已确认 SAA 上，在 SAA 区间内权重守恒，锁定安全层并强制复核和失效。
- 现有配置引擎：继续保存用户私有人民币研究资金池，优先锁定流动性与未来现金需求，再用六类审核基金/ETF的完整共同月度复权净值做受约束等风险贡献；本次框架建设不新增优化算法选择器，也不改变 ERC 行为。配置版本不可覆盖，外汇只披露为底层币种敞口，期货和期权只作为实施与对冲工具。
...
- `opportunity-worker`：后台物化跨资产机会快照；请求接口只读，不在页面打开时抓取或刷新数据。
