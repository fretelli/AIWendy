...
export type AgentSession = { id: string; agent_definition_id?: string; title: string; status: string; summary?: string; context_tokens: number; is_pinned: boolean; archived_at?: string; last_message_at: string; created_at: string }
...
export const agentPlatformApi = {
  ...
  createSession: (body: { agent_definition_id: string; title?: string }) => apiJson<AgentSession>(`${base}/sessions`, { method: 'POST', body }),
  updateSession: (id: string, body: { title?: string; is_pinned?: boolean; archived?: boolean }) => apiJson<AgentSession>(`${base}/sessions/${id}`, { method: 'PATCH', body }),
