...
export type MarketAnalysisMetadata = { status: "available" | "unavailable" | string; reason_code?: string; as_of?: string; coverage?: number; history_coverage?: number; universe_coverage?: { broad?: { expected: number; available: number; coverage: number; missing_codes: string[] }; sw_l1?: { available: number } }; publication_version: string; capability_version: string; methodology_key: string; source_datasets: string[]; materialization_version?: string; computed_at?: string; source_watermarks?: Record<string, string | null> };
export type ValuationMethodology = { cross_universe_comparable: false; pe: { broad: { basis: "ttm"; source_table: string; source_field: "pe_ttm" }; sw_l1: { basis: "provider_defined"; source_table: string; source_field: "pe" } }; pb: { basis: "provider_defined"; source_fields: Record<"broad" | "sw_l1", string> }; percentile: { window: "5Y"; comparison: "same_code_history" }; crowding: { methodology_key: string; independent_of_display_pe: true; weights: Record<string, number>; valuation_expansion_priority: Array<"pe_ttm" | "pb" | "ps_ttm">; coverage_threshold: number } };
export type ValuationItem = { code: string; name: string; trade_date: string; pe?: number; pb?: number; total_mv?: number; turnover_rate?: number; source: string; universe?: "broad" | "sw_l1"; classification_source?: string; classification_level?: string; industry_code?: string; pe_basis?: "ttm" | "provider_defined" | string; pe_source_field?: string; pb_basis?: "provider_defined" | string; pb_source_field?: string; comparison_group?: "broad_pe_ttm" | "sw_l1_provider_pe" | string; pe_percentile?: number; pb_percentile?: number; activity_percentile?: number; crowding_percentile?: number | null; crowding_status?: "available" | "unavailable" | string; crowding_reason?: string; crowding_coverage?: number; crowding_coverage_actual?: number; crowding_coverage_expected?: number; crowding_coverage_basis?: string; crowding_threshold?: number; crowding_methodology_key?: string; percentile_change_1m?: number; percentile_change_3m?: number; constituent_weight_basis?: "official_index_weight" | "derived_free_float_weight" | string; constituent_status?: "available" | "unavailable" | string; constituent_reason?: string | null; top_constituents?: Array<{ code: string; name: string; weight: number; weight_basis: string }> };
export type ValuationBoard = { metadata: MarketAnalysisMetadata; percentile_window: string; methodology: ValuationMethodology; cross_universe_comparable: false; point_in_time?: true; synthetic_substitution: false; items: ValuationItem[] };
export type ValuationHistory = { metadata: MarketAnalysisMetadata; code: string; universe: "broad" | "sw_l1"; requested_limit: number; available_points_total: number; available_start?: string; available_end?: string; methodology: ValuationMethodology; cross_universe_comparable: false; point_in_time: true; synthetic_substitution: false; points: Array<{ as_of: string; analysis_version: string; pe?: number; pb?: number; pe_basis?: string; pe_source_field?: string; pb_basis?: string; pb_source_field?: string; comparison_group?: string; pe_percentile?: number; pb_percentile?: number; activity_percentile?: number; crowding_percentile?: number | null; crowding_status?: string; crowding_reason?: string; crowding_coverage?: number; percentile_change_1m?: number; percentile_change_3m?: number }> };
export type HeldIndustries = { metadata: MarketAnalysisMetadata; industry_codes: string[]; industries: Array<{ code: string; name: string }>; position_symbols: string[]; mapped_symbols: Record<string, { code: string; name: string; stock_name?: string }>; missing_symbols: string[]; coverage: number; covered: number; eligible: number };
...
export const marketsApi = {
  ...
  valuationHistory: (code: string, universe: "broad" | "sw_l1", limit = 252) => apiJson<ValuationHistory>(`${marketsBase}/valuation/history?code=${encodeURIComponent(code)}&universe=${universe}&limit=${limit}`),
  heldIndustries: () => apiJson<HeldIndustries>(`${marketsBase}/valuation/held-industries`),
  correlations: (window = 60) => apiJson<CorrelationBoard>(`${marketsBase}/correlations?window=${window}`),
  correlationHistory: (window = 60, limit = 756) => apiJson<CorrelationHistory>(`${marketsBase}/correlations/history?window=${window}&limit=${limit}`),
