...
export function RatesDrilldown() {
  ...
  return <Panel><SectionTitle title={locale === "zh" ? "利率与债券" : "Rates & Bonds"} en="PROVIDER-NATIVE SERIES" />
    {catalog?.items.length ? <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">{catalog.items.map((item) => <div key={item.key} className="rounded-md border border-agent-border bg-agent-raised p-3"><div className="flex items-center gap-2"><StatusDot status={item.available ? "complete" : "unavailable"} /><span className="text-xs text-agent-text">{item.label}</span></div><p className="mt-3 font-data text-[9px] text-agent-dim">{item.start || "—"} → {item.end || "—"} · {item.points || 0} PTS</p>{item.unavailable_reason ? <p className="mt-2 text-[10px] leading-5 text-agent-amber">{item.unavailable_reason}</p> : null}</div>)}</div> : <EmptyPanel title={locale === "zh" ? "利率目录不可用" : "Rates catalog unavailable"} detail={locale === "zh" ? "yc_cb 等缺失来源会保持明确不可用，不使用期货或合成曲线。" : "Missing sources such as yc_cb remain explicitly unavailable without futures or synthetic substitutes."} />}
  </Panel>;
}
