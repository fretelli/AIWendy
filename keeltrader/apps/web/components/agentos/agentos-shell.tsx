      ...
      <section className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-[60px] shrink-0 items-center gap-4 border-b border-agent-border bg-agent-chrome px-4 lg:px-[26px]">
          ...
          <div className="hidden overflow-hidden rounded-md border border-agent-border md:flex">
            {["1M", "3M", "1Y", "3Y"].map((value) => <button type="button" key={value} onClick={() => setPeriod(value)} aria-pressed={period === value} className={`px-3 py-1.5 font-data text-[10px] ${period === value ? "bg-agent-mint text-agent-canvas" : "text-agent-dim hover:text-agent-text"}`}>{value}</button>)}
          </div>
