  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
    <DialogContent className="h-[min(920px,calc(100dvh-24px))] w-[min(1280px,calc(100vw-24px))] max-w-none overflow-y-auto border-agent-border bg-agent-surface text-agent-text">
      ...
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Metric label={locale === "zh" ? "PE 分位" : "PE percentile"} value={percent(item.pe_percentile)} />
        <Metric label={locale === "zh" ? "PB 分位" : "PB percentile"} value={percent(item.pb_percentile)} />
      ...
      <div className="grid gap-3 lg:grid-cols-[1.2fr_1fr]">
        ...
        <section className="rounded-sm border border-agent-border bg-agent-raised p-3"><h3 className="mb-2 text-xs text-agent-text">{locale === "zh" ? "来源与方法" : "Source & Methodology"}</h3><dl className="grid grid-cols-[92px_1fr] gap-x-3 gap-y-2 text-[10px]"><dt className="text-agent-dim">As of</dt><dd className="font-data text-agent-muted">{item.trade_date}</dd><dt className="text-agent-dim">Method</dt><dd className="font-data text-agent-muted">kt_valuation_percentile_v2</dd><dt className="text-agent-dim">Source</dt><dd className="font-data text-agent-muted">{item.source}</dd><dt className="text-agent-dim">Universe</dt><dd className="font-data text-agent-muted">{universe}</dd><dt className="text-agent-dim">Weight</dt><dd className="font-data text-agent-muted">{item.constituent_weight_basis || "—"}</dd><dt className="text-agent-dim">Crowding</dt><dd className="font-data text-agent-muted">{item.crowding_methodology_key || "kt_market_crowding_v3"}</dd></dl><p className="mt-3 text-[9px] leading-4 text-agent-dim">{locale === "zh" ? "估值分位基于各代码截至当日的五年真实历史；宽基成分使用官方指数权重，申万一级使用自由流通市值推导权重并明确标记。拥挤度覆盖低于 80% 时保持不可用。" : "Percentiles use each code's real five-year history through the as-of date. Broad indices use official weights; SW L1 uses explicitly labelled derived free-float weights. Crowding remains unavailable below 80% coverage."}</p></section>
      </div>
    </DialogContent>
  </Dialog>;
}
