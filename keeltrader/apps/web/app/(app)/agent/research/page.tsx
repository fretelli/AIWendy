...
export default function ResearchPage() {
  ...
  const [tab, setTab] = useUrlTab(["hypotheses", "judgments", "consensus", "reports"], "hypotheses");
...
function ReportDialog({ open, setOpen, reload, locale }: { open: boolean; setOpen: (value: boolean) => void; reload: () => Promise<void>; locale: string }) {
  const [form, setForm] = useState({ title: "", citation: "", zh: "", en: "" }); const [saving, setSaving] = useState(false);
  const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); try { const asOf = new Date().toISOString().slice(0, 10); const document = await agentOSApi.createDocument({ title: form.title, document_type: "research_note" }); await agentOSApi.generateDocument(document.id, { bodies: { "zh-CN": form.zh, "en-US": form.en }, structured: {}, source_snapshot: { as_of: asOf, generated_at: new Date().toISOString(), source: "user_confirmed_research_note", citations: [{ label: form.citation, as_of: asOf }] } }); await reload(); setOpen(false); toast.success(locale === "zh" ? "中英文 PDF 已生成" : "Chinese and English PDFs generated"); } catch (error) { toast.error(error instanceof Error ? error.message : "Generation failed"); } finally { setSaving(false); } };
  return <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-w-3xl"><DialogHeader><DialogTitle>{locale === "zh" ? "生成双语研究报告" : "Generate bilingual research report"}</DialogTitle><DialogDescription>{locale === "zh" ? "两种语言共享同一事实快照并分别生成真正 PDF；必须记录截至日和可追溯资料。" : "Both languages share one fact snapshot and produce real PDF files; an as-of date and traceable source are required."}</DialogDescription></DialogHeader><form onSubmit={submit} className="flex flex-col gap-4"><div><Label htmlFor="report-title">{locale === "zh" ? "报告标题" : "Report title"}</Label><Input id="report-title" className="mt-2" value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} required /></div><div><Label htmlFor="report-citation">{locale === "zh" ? "资料引用（标题、报告 ID 或链接）" : "Source citation (title, report ID, or link)"}</Label><Input id="report-citation" className="mt-2" value={form.citation} onChange={(event) => setForm({ ...form, citation: event.target.value })} required /></div><div className="grid gap-4 md:grid-cols-2"><div><Label htmlFor="report-zh">中文</Label><Textarea id="report-zh" className="mt-2 min-h-52" value={form.zh} onChange={(event) => setForm({ ...form, zh: event.target.value })} required /></div><div><Label htmlFor="report-en">English</Label><Textarea id="report-en" className="mt-2 min-h-52" value={form.en} onChange={(event) => setForm({ ...form, en: event.target.value })} required /></div></div><DialogFooter><Button type="submit" disabled={saving}>{saving ? (locale === "zh" ? "生成中…" : "Generating…") : (locale === "zh" ? "生成版本" : "Generate version")}</Button></DialogFooter></form></DialogContent></Dialog>;
}
