...
const sections = [
  ...
  { href: "/agent/market/rates", label: "利率债券", icon: Landmark },
...
export function MarketShell({
  ...
  return (
    <div className="h-full min-h-0 overflow-y-auto bg-background/80">
      <header className="research-bearing sticky top-0 z-30 border-b bg-card/95 shadow-sm backdrop-blur">
        <div className="flex min-h-16 items-center gap-2 px-3 sm:px-5">
          <div className="hidden border-r pr-4 sm:block">
            <KeelMark />
          </div>
          ...
          <Button asChild size="sm" variant="outline">
            <Link href="/agent/holders">
              <Radar className="mr-1.5 h-4 w-4" />
              <span className="hidden md:inline">股东雷达</span>
            </Link>
          </Button>
          <Button asChild size="sm" variant="outline">
            <Link href="/agent">
              <ShipWheel className="mr-1.5 h-4 w-4" />
              <span className="hidden md:inline">研究台</span>
            </Link>
          </Button>
          <ThemeMenu />
        </div>
        ...
        {trail && (
          <div className="evidence-rail flex min-h-8 items-center gap-3 overflow-x-auto border-t px-4 text-[9px] text-muted-foreground">
            ...
            <span className="shrink-0 text-foreground">{trail.object}</span>
            <span className="shrink-0">{trail.asOf || "日期不可用"}</span>
...
export function MarketNavigation({
  ...
  return (
    <nav
      aria-label="市场数据分区"
      className="flex overflow-x-auto border-t bg-card/95 px-3 sm:px-5"
    >
      {sections.map(({ href, label, icon: Icon }) => {
        ...
        return (
          <Link
            key={href}
            href={href}
            className={`relative flex shrink-0 items-center gap-2 px-4 py-3 text-xs transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring ${active ? "text-foreground" : "text-muted-foreground hover:text-foreground"}`}
          >
            <Icon className="h-3.5 w-3.5" />
            {label}
            {active && (
              <span className="absolute inset-x-3 bottom-0 h-0.5 bg-[hsl(var(--copper-foreground))]" />
            )}
          </Link>
        );
      })}
...
export function DataLedger({
  ...
  return (
    <div className="grid gap-px overflow-hidden rounded-xl border bg-border text-[10px] sm:grid-cols-4">
      <LedgerCell label="来源" value={source} />
      <LedgerCell
        label="数据范围"
        value={start && end ? `${start} — ${end}` : "不可用"}
      />
      <LedgerCell
        label="数据点"
        value={points === undefined ? "—" : String(points)}
      />
      <LedgerCell label="覆盖口径" value={scope} />
    </div>
  );
}
...
function LedgerCell({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0 bg-card/90 px-4 py-3">
      <p className="text-muted-foreground">{label}</p>
      <p className="mt-1 truncate font-data text-foreground" title={value}>
        {value}
      </p>
    </div>
  );
