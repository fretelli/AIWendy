...
export function NativeSeriesChart({ dates, series, height = 430, valueLabel = formatNumber }: {
  ...
  return <div className="overflow-hidden rounded-2xl border bg-card/88 shadow-sm">
    <div ref={host} data-chart-canvas="raw-market-series" className="relative w-full p-2 md:p-5" style={{ height }}>
...
function ChartSvg({ width, height, dates, series, hover, onHover, valueLabel }: { width: number; height: number; dates: string[]; series: PlotSeries[]; hover: number | null; onHover: (value: number | null) => void; valueLabel: (value: number) => string }) {
  ...
  const pointerMove = (event: ReactPointerEvent<SVGRectElement>) => { const rect = event.currentTarget.getBoundingClientRect(); const ratio = Math.max(0, Math.min(1, (event.clientX - rect.left - margin.left) / plotWidth)); onHover(Math.round(ratio * Math.max(0, dates.length - 1))) }
  return <>
    <svg data-chart-series="raw-market-series" role="img" aria-label="原始市场数据历史图" width={width} height={height} viewBox={`0 0 ${width} ${height}`} className="block">
      ...
      <rect x={margin.left} y={margin.top} width={plotWidth} height={plotHeight} fill="transparent" onPointerMove={pointerMove} onPointerLeave={() => onHover(null)} />
    </svg>
    {hovered && hover !== null && <div className="pointer-events-none absolute top-6 z-10 min-w-48 rounded-xl border bg-popover/96 p-3 text-xs shadow-xl backdrop-blur" style={{ left: Math.max(6, Math.min(width - 205, x(hover) + 12)) }}><p className="font-data font-semibold">{hovered.date}</p><div className="mt-2 space-y-1.5">{hovered.rows.map(row => <div key={row.key} className="flex items-center justify-between gap-5"><span className="flex items-center gap-2 text-muted-foreground"><i className="h-2 w-2 rounded-full" style={{ background: row.color }} />{row.label}</span><span className="font-data">{typeof row.value === 'number' ? valueLabel(row.value) : '—'}</span></div>)}</div></div>}
  </>
}
