...
function RawLinks({ item }: { item: Opportunity }) { const href = item.domain === "macro" ? "/agent/market/macro" : item.domain === "rates" ? "/agent/market/rates" : item.domain === "futures" ? "/agent/market/futures" : item.domain === "options" ? "/agent/market/options" : item.domain === "capital" ? "/agent/market/capital" : item.domain === "holder" ? "/agent/holders" : "/agent"; return <section className="mt-6 rounded-xl border border-dashed p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-xs font-semibold">源数据与完整历史</h3><p className="mt-1 text-[10px] text-muted-foreground">图表引用 {item.chart_refs?.length || 0} 个；进入对应数据台查看当前数据库全部可用原始历史。</p></div><Button asChild size="sm" variant="outline"><Link href={href}>打开源数据历史</Link></Button></div></section>; }
