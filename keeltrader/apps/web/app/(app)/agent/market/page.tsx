...
export default function MarketPage() {
  ...
  const { data: macro = null } = useSWR<MacroCatalog>(showMacro ? "markets/macro/catalog-v2" : null, marketsApi.macroCatalog);
...
function MacroBoard({ data, locale }: { data: MacroCatalog | null; locale: "zh" | "en" }) {
  ...
  const { data: detail = null, isLoading, error } = useSWR<MacroSeriesDetail>(
    selected ? `markets/macro/series/${selected}/analysis-v2` : null,
  ...
  const themes = ["growth", "prices", "credit", "sentiment", "external", "fiscal"] as const;
  return <div className="flex min-h-[calc(100dvh-104px)] flex-col gap-3"><Panel><PanelTitle title={locale === "zh" ? "中国核心宏观全景" : "China Core Macro Dashboard"} note={locale === "zh" ? "结构化 Tushare 优先 · eco_cal 白名单补齐 · 禁止代理与合成" : "Structured Tushare first · gated eco_cal fallback · no proxies"} />
    {rows.length ? <div className="grid gap-3 xl:grid-cols-2">{themes.map(theme => <section key={theme} className="rounded-sm border border-agent-border bg-agent-raised/30 p-3"><div className="mb-2 flex items-center justify-between"><h3 className="text-[11px] font-medium text-agent-text">{macroThemeLabel(theme, locale)}</h3><span className="font-data text-[8px] text-agent-dim">{rows.filter(row => row.theme === theme && row.available).length}/{rows.filter(row => row.theme === theme).length}</span></div><div className="grid gap-2 sm:grid-cols-2">{rows.filter(row => row.theme === theme).map(row => {
      ...
      return <button type="button" disabled={!row.available} onClick={() => { setSelected(row.key); setSelectedField(null); }} key={row.key} className={cn("min-h-[134px] rounded-sm border border-agent-border bg-agent-surface p-3 text-left transition-colors", row.available ? "hover:border-agent-blue/50 hover:bg-agent-raised focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-agent-mint" : "cursor-default opacity-75")}><div className="flex items-start justify-between gap-2"><div><p className="text-[11px] text-agent-text">{macroLabel(row.key, locale)}</p><p className="mt-1 font-data text-[8px] text-agent-dim">{macroSourceNote(row, locale)}</p></div><MacroMetricCell metric={summary?.primary} locale={locale} emphasis /></div>{row.available ? <><div className="mt-3 grid grid-cols-3 gap-2 border-t border-agent-border pt-2"><div><p className="font-data text-[7px] text-agent-dim">{macroSequentialLabel(row.key, locale)}</p><MacroMetricCell metric={summary?.mom} locale={locale} signed /></div><div><p className="font-data text-[7px] text-agent-dim">{locale === "zh" ? "同比比较" : "YoY compare"}</p><MacroMetricCell metric={summary?.yoy} locale={locale} signed /></div><div><p className="font-data text-[7px] text-agent-dim">{locale === "zh" ? "10Y 分位" : "10Y pct."}</p><MacroMetricCell metric={summary?.percentile_10y} locale={locale} percentile /></div></div><div className="mt-2 flex items-end gap-3"><div className="h-7 flex-1">{sparkline.length > 1 ? <MiniLine values={sparkline} color="var(--agent-blue)" height={30} /> : null}</div><span className="font-data text-[7px] text-agent-dim">{row.end || "—"}</span></div></> : <p className="mt-4 text-[9px] leading-4 text-agent-amber">{row.reason || (locale === "zh" ? "数据未接入" : "Not integrated")}</p>}</button>;
    })}</div></section>)}</div> : <DataGap title={locale === "zh" ? "宏观数据不可用" : "Macro data unavailable"} locale={locale} />}
  </Panel><Panel className="grid grid-cols-[70px_1fr] items-start gap-4 py-3"><span className="font-data text-[9px] text-agent-mint">METHOD<br/>{locale === "zh" ? "口径" : "READ"}</span><p className="text-[10px] leading-5 text-agent-muted">{locale === "zh" ? "首页只展示专业主序列：GDP、CPI、PPI、M2 使用官方同比，PMI 使用指数水平；社融和新增贷款不做误导性月环比，历史分位仅与过去十年同历月比较。累计值和 PMI 细项放在下钻。财政缺口保持未接入。" : "Headline cards use professional primary series. Seasonal credit flows do not show misleading MoM changes and are ranked only against the same calendar month. Cumulative and PMI subseries remain in drilldown; fiscal gaps stay explicit."}</p></Panel>
    <Dialog open={Boolean(selected)} onOpenChange={(open) => { if (!open) { setSelected(null); setSelectedField(null); } }}><DialogContent className="h-[min(920px,calc(100dvh-24px))] w-[min(1380px,calc(100vw-24px))] max-w-none overflow-y-auto border-agent-border bg-agent-surface text-agent-text"><DialogHeader><DialogTitle>{selected ? macroLabel(selected, locale) : "—"}</DialogTitle><DialogDescription>{selectedRow ? `${macroSourceNote(selectedRow, locale)} · ${selectedRow.start || "—"} → ${selectedRow.end || "—"} · ${selectedRow.points || 0} ${locale === "zh" ? "个观测" : "observations"}` : ""}</DialogDescription></DialogHeader><HistoryRangeToggle value={range} values={MACRO_RANGES} onChange={(value) => setRange(value as MacroRange)} locale={locale} />{isLoading ? <EmptyPanel title={locale === "zh" ? "正在读取正式历史" : "Loading formal history"} detail={locale === "zh" ? "只加载当前指标。" : "Only the selected indicator is loaded."} /> : error || !detail?.series ? <DataGap title={locale === "zh" ? "宏观序列不可用" : "Macro series unavailable"} reason={detail?.reason_code} locale={locale} /> : <><div className="grid gap-3 lg:grid-cols-2"><MacroChartPanel kind="primary" detail={detail} range={range} locale={locale} /><MacroChartPanel kind="mom" detail={detail} range={range} locale={locale} /><MacroChartPanel kind="yoy" detail={detail} range={range} locale={locale} /><MacroChartPanel kind="percentile_10y" detail={detail} range={range} locale={locale} /></div>{detail.latest_release ? <Panel tone="raised" className="mt-3"><PanelTitle title={locale === "zh" ? "最新发布与预期差" : "Latest Release & Surprise"} note={detail.latest_release.quality_state || undefined} /><div className="grid gap-3 text-[10px] sm:grid-cols-4"><MacroReleaseValue label={locale === "zh" ? "实际" : "Actual"} value={detail.latest_release.actual} /><MacroReleaseValue label={locale === "zh" ? "预期" : "Forecast"} value={detail.latest_release.forecast} /><MacroReleaseValue label={locale === "zh" ? "预期差" : "Surprise"} value={detail.latest_release.surprise} /><div><p className="text-agent-dim">{locale === "zh" ? "发布日期" : "Released"}</p><p className="mt-1 font-data text-agent-muted">{detail.latest_release.release_date || "—"}</p></div></div></Panel> : null}<MacroFieldDrilldown detail={detail} fieldDetail={fieldDetail} selectedField={selectedField} setSelectedField={setSelectedField} range={range} locale={locale} loading={fieldLoading} /></>}</DialogContent></Dialog>
  </div>;
}
...
function MacroFieldDrilldown({ detail, fieldDetail, selectedField, setSelectedField, range, locale, loading }: { detail: MacroSeriesDetail; fieldDetail: MacroSeriesDetail | null; selectedField: string | null; setSelectedField: (field: string | null) => void; range: MacroRange; locale: "zh" | "en"; loading: boolean }) {
  ...
  return <Panel tone="raised" className="mt-3"><PanelTitle title={locale === "zh" ? "细分数据" : "Detailed Subseries"} note={locale === "zh" ? "仅开放显式字段白名单" : "Explicit field allowlist only"} /><div className="grid gap-3 lg:grid-cols-[320px_1fr]"><div className="max-h-[360px] overflow-y-auto pr-1">{groups.map(group => <div key={group} className="mb-3"><p className="mb-1 font-data text-[8px] text-agent-dim">{group}</p><div className="flex flex-wrap gap-1">{fields.filter(field => field.group === group).map(field => <button type="button" key={field.key} onClick={() => setSelectedField(field.key)} className={cn("rounded-sm border px-2 py-1 text-[9px]", selectedField === field.key ? "border-agent-mint bg-agent-mint text-agent-canvas" : "border-agent-border text-agent-muted hover:bg-agent-raised")}>{field.label}</button>)}</div></div>)}</div><div>{loading ? <EmptyPanel title={locale === "zh" ? "正在读取细分序列" : "Loading subseries"} detail="" /> : selectedField && fieldDetail ? <><p className="mb-2 text-[9px] leading-4 text-agent-amber">{selectedField === "gdp" || selectedField === "pi" || selectedField === "si" || selectedField === "ti" ? (locale === "zh" ? "年内累计值只用于观察规模和结构，不能跨季度直接计算普通历史分位。" : "Year-to-date cumulative values are for scale and structure only, not ordinary cross-quarter percentiles.") : locale === "zh" ? `${fieldDetail.field_meta?.label || selectedField} · 官方原始字段` : `${fieldDetail.field_meta?.label || selectedField} · official raw field`}</p>{rows.some(row => row.value != null) ? <TimeSeriesChart dates={rows.map(row => row.period)} series={[{ name: fieldDetail.field_meta?.label || selectedField, values: rows.map(row => row.value) }]} locale={locale} title={`${fieldDetail.field_meta?.label || selectedField} · ${range}`} height={290} /> : <DataGap title={locale === "zh" ? "细分历史不可用" : "Subseries unavailable"} locale={locale} />}</> : <EmptyPanel title={locale === "zh" ? "选择一个细分指标" : "Select a subseries"} detail={locale === "zh" ? "GDP、CPI、货币、社融和 PMI 的正式细项均在这里切换。" : "Switch among official GDP, CPI, money, credit and PMI fields here."} />}</div></div></Panel>;
}
...
function macroLabel(key: string, locale: "zh" | "en") { const labels: Record<string, [string, string]> = { gdp: ["GDP 同比", "GDP YoY"], cpi: ["CPI 同比", "CPI YoY"], ppi: ["PPI 同比", "PPI YoY"], money_supply: ["M2 同比", "M2 YoY"], social_financing: ["社会融资增量", "Social Financing Flow"], pmi: ["制造业 PMI", "Manufacturing PMI"], industrial_production_yoy: ["工业增加值同比", "Industrial Production YoY"], retail_sales_yoy: ["社会消费品零售同比", "Retail Sales YoY"], fixed_asset_investment_ytd_yoy: ["固定资产投资累计同比", "Fixed Asset Investment YTD YoY"], urban_unemployment_rate: ["城镇调查失业率", "Urban Unemployment"], exports_usd_yoy: ["出口同比（美元）", "Exports YoY (USD)"], imports_usd_yoy: ["进口同比（美元）", "Imports YoY (USD)"], trade_balance_usd: ["贸易差额", "Trade Balance"], fx_reserves_usd: ["外汇储备", "FX Reserves"], new_rmb_loans: ["新增人民币贷款", "New RMB Loans"], fiscal: ["财政收支", "Fiscal Revenue & Expenditure"], shibor: ["上海银行间同业拆放利率", "SHIBOR"], lpr: ["贷款市场报价利率", "LPR"], us_treasury: ["美国国债收益率", "US Treasury"], us_real_treasury: ["美国实际国债收益率", "US Real Treasury"] }; return labels[key]?.[locale === "zh" ? 0 : 1] || key; }
function macroThemeLabel(theme: string, locale: "zh" | "en") { const labels: Record<string, [string, string]> = { growth: ["增长与需求", "Growth & Demand"], prices: ["物价", "Prices"], credit: ["信用与货币", "Credit & Money"], sentiment: ["景气与就业", "Business Climate & Jobs"], external: ["外贸与外储", "Trade & Reserves"], fiscal: ["财政", "Fiscal"] }; return labels[theme]?.[locale === "zh" ? 0 : 1] || theme; }
function macroSourceNote(row: MacroCatalog["items"][number], locale: "zh" | "en") { if (row.quality?.source_type === "eco_cal_gated") return locale === "zh" ? `Tushare eco_cal · 覆盖门禁 ${row.quality.coverage_points || 0}/${row.quality.minimum_samples || 0}` : `Tushare eco_cal · coverage gate ${row.quality.coverage_points || 0}/${row.quality.minimum_samples || 0}`; if (row.quality?.source_type === "unavailable") return locale === "zh" ? "未接入 · 不使用代理" : "Not integrated · no proxy"; return locale === "zh" ? `Tushare 结构化 · ${row.source || row.table || "—"}` : `Structured Tushare · ${row.source || row.table || "—"}`; }
