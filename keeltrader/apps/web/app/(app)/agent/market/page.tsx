...
import { agentPlatformApi, marketsApi, type MacroMarketSnapshot, type MarketCapitalSnapshot } from "@/lib/api/agent-platform";
...
export default function MarketPage() {
  ...
  useEffect(() => { void Promise.all([marketsApi.capital(), agentPlatformApi.macroMarket()]).then(([nextCapital, nextMacro]) => { setCapital(nextCapital); setMacro(nextMacro); }).catch(() => undefined); }, []);
  const breadth = capital?.breadth;
  const history = capital?.history ?? [];
  const macroRows = useMemo(() => Object.entries(macro?.series ?? {}), [macro]);
  const defaultTab = ["valuation", "correlation", "factors", "macro", "capital"].includes(params.get("tab") || "") ? params.get("tab")! : "valuation";
  return <DashboardPage>
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      ...
      <MetricCard label="7D FUNDING" value={capital?.funding_rates.seven_day_pct !== undefined ? `${capital.funding_rates.seven_day_pct.toFixed(2)}%` : "—"} note={capital?.funding_rates.as_of} color="text-agent-blue" />
    </div>
    <Tabs key={defaultTab} defaultValue={defaultTab} className="flex flex-col gap-3">
      <TabsList className="h-auto w-fit max-w-full overflow-x-auto border border-agent-border bg-agent-chrome p-1"><TabsTrigger value="valuation">{locale === "zh" ? "估值" : "Valuation"}</TabsTrigger><TabsTrigger value="correlation">{locale === "zh" ? "相关性" : "Correlation"}</TabsTrigger><TabsTrigger value="factors">{locale === "zh" ? "因子" : "Factors"}</TabsTrigger><TabsTrigger value="macro">{locale === "zh" ? "宏观" : "Macro"}</TabsTrigger><TabsTrigger value="capital">{locale === "zh" ? "资金" : "Capital"}</TabsTrigger></TabsList>
