...
export default function HoldingsPage() {
  ...
  const byAsset = useMemo(() => {
    ...
    for (const item of valuation?.positions ?? []) result.set(item.asset_class, (result.get(item.asset_class) || 0) + (item.market_value || 0));
  ...
  return <DashboardPage>
    ...
    {!accounts.length ? <Panel><EmptyPanel title={locale === "zh" ? "建立真实组合账本" : "Create a real portfolio ledger"} detail={locale === "zh" ? "KeelTrader 不连接券商。请创建账户后手工录入，或导入持仓、流水和历史净值 CSV。" : "KeelTrader does not connect to brokers. Create an account, then add data manually or import CSV."} action={locale === "zh" ? "创建组合账户" : "Create account"} onAction={() => void createAccount()} /></Panel> : <>
      ...
      <Tabs defaultValue="detail" className="flex flex-col gap-3"><TabsList className="h-auto w-fit border border-agent-border bg-agent-chrome p-1"><TabsTrigger value="detail">{locale === "zh" ? "持仓明细" : "Positions"}</TabsTrigger><TabsTrigger value="hedge">{locale === "zh" ? "对冲与衍生品" : "Hedge & Derivatives"}</TabsTrigger></TabsList>
        <TabsContent value="detail" className="mt-0 grid gap-3 xl:grid-cols-[280px_1fr]">
          ...
          <Panel className="overflow-hidden p-0"><div className="p-4"><SectionTitle title={locale === "zh" ? "持仓明细" : "Position Details"} en="COST / PRICE / RISK" /></div>{valuation?.positions.length ? <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-[10px]"><thead className="border-y border-agent-border bg-agent-chrome font-data text-agent-dim"><tr><th className="px-4 py-2 font-normal">{locale === "zh" ? "标的" : "Instrument"}</th><th className="px-3 py-2 font-normal">{locale === "zh" ? "数量" : "Quantity"}</th><th className="px-3 py-2 font-normal">{locale === "zh" ? "成本" : "Cost"}</th><th className="px-3 py-2 font-normal">{locale === "zh" ? "价格" : "Price"}</th><th className="px-3 py-2 font-normal">{locale === "zh" ? "市值" : "Market value"}</th><th className="px-3 py-2 font-normal">P&L</th><th className="px-3 py-2 font-normal">SOURCE</th></tr></thead><tbody className="divide-y divide-agent-border">{valuation.positions.map((item) => <tr key={item.instrument_id} className="hover:bg-agent-raised"><td className="px-4 py-3"><span className="block text-xs text-agent-text">{item.name}</span><span className="font-data text-[9px] text-agent-dim">{item.symbol} · {item.asset_class}</span></td><td className="px-3 py-3 font-data text-agent-muted">{formatNumber(item.quantity)}</td><td className="px-3 py-3 font-data text-agent-muted">{item.average_cost ? formatNumber(item.average_cost) : "—"}</td><td className="px-3 py-3 font-data text-agent-text">{item.price ? formatNumber(item.price) : "—"}</td><td className="px-3 py-3 font-data text-agent-text">{item.market_value !== undefined ? formatCurrency(item.market_value, item.currency) : "—"}</td><td className={`px-3 py-3 font-data ${(item.unrealized_pnl || 0) >= 0 ? "text-agent-up" : "text-agent-down"}`}>{item.unrealized_pnl !== undefined ? formatCurrency(item.unrealized_pnl, item.currency) : "—"}</td><td className="px-3 py-3 font-data text-[9px] uppercase text-agent-dim">{item.price_source || "missing"}<br/>{item.price_date || "—"}</td></tr>)}</tbody></table></div> : <div className="p-4"><EmptyPanel title={locale === "zh" ? "组合为空" : "Portfolio is empty"} detail={locale === "zh" ? "生产环境不会自动填入示例持仓。" : "Production never inserts sample positions."} /></div>}</Panel>
        </TabsContent>
