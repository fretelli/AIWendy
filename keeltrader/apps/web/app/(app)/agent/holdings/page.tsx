...
import { agentOSApi, type HoldingDetail, type PortfolioAccount, type PortfolioAnalytics, type PortfolioValuation } from "@/lib/api/agentos";
...
export default function HoldingsPage() {
  ...
  const [form, setForm] = useState<PositionForm>({ symbol: "", name: "", market: "CN", asset_class: "stock", instrument_type: "stock", provider_symbol: "", direction: "long", multiplier: "1", quantity: "", price: "", trade_date: new Date().toISOString().slice(0, 10), currency: "CNY" });
  ...
  const loadAccount = async (id: string) => {
    const [value, flows, nextAnalytics] = await Promise.all([agentOSApi.valuation(id), agentOSApi.transactions(id), agentOSApi.analytics(id)]);
    setValuation(value); setTransactions(flows.items); setAnalytics(nextAnalytics);
  };
  useEffect(() => {
    let cancelled = false;
    void (async () => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void (async () => {
      const [value, flows, nextAnalytics] = await Promise.all([agentOSApi.valuation(accountId), agentOSApi.transactions(accountId), agentOSApi.analytics(accountId)]);
      if (cancelled) return;
      setValuation(value);
  ...
  const savePosition = async (event: FormEvent) => {
    ...
    try {
      await agentOSApi.addTransaction(accountId, { ...form, transaction_type: "opening", quantity: Number(form.quantity), price: Number(form.price), manual_price: Number(form.price), multiplier: Number(form.multiplier), cash_amount: 0 });
  ...
  return <DashboardPage>
    ...
    {!accounts.length ? <Panel><EmptyPanel title={locale === "zh" ? "建立真实组合账本" : "Create a real portfolio ledger"} detail={locale === "zh" ? "KeelTrader 不连接券商。请创建账户后手工录入，或导入持仓、流水和历史净值 CSV。" : "KeelTrader does not connect to brokers. Create an account, then add data manually or import CSV."} action={locale === "zh" ? "创建组合账户" : "Create account"} onAction={() => void createAccount()} /></Panel> : <>
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard label="TODAY P&L" value={analytics?.today_pnl.value != null ? formatCurrency(analytics.today_pnl.value, valuation?.base_currency || "CNY") : "—"} note={analytics?.today_pnl.as_of || analytics?.today_pnl.reason} color={(analytics?.today_pnl.value || 0) >= 0 ? "text-agent-up" : "text-agent-down"} />
        <MetricCard label="CASH" value={analytics?.cash.value != null ? formatCurrency(analytics.cash.value, valuation?.base_currency || "CNY") : "—"} note={`${transactions.length} ${locale === "zh" ? "条不可变流水" : "immutable entries"}`} color="text-agent-blue" />
        <MetricCard label="VOLATILITY" value={analytics?.volatility.value != null ? `${(analytics.volatility.value * 100).toFixed(1)}%` : "—"} note={analytics?.volatility.reason || `${analytics?.volatility.observations || 0} observations`} color="text-agent-amber" />
      </div>
      {valuation ? <MissingData items={valuation.missing} /> : null}
      <Tabs value={tab} onValueChange={setTab} className="flex flex-col gap-3"><TabsList className="h-auto w-fit border border-agent-border bg-agent-chrome p-1 lg:hidden"><TabsTrigger value="detail">{locale === "zh" ? "持仓明细" : "Positions"}</TabsTrigger><TabsTrigger value="hedge">{locale === "zh" ? "对冲与衍生品" : "Hedge & Derivatives"}</TabsTrigger></TabsList>
        <TabsContent value="detail" className="mt-0 grid gap-3 xl:grid-cols-[280px_1fr]">
          ...
          {valuation?.positions.length ? <Panel className="xl:col-span-2"><SectionTitle title={locale === "zh" ? "标的详情抽屉" : "Instrument Detail Drawer"} en="HISTORY / FUNDAMENTALS / RISK" /><div className="flex flex-wrap gap-2">{valuation.positions.map((item) => <Button key={item.instrument_id} variant="outline" size="sm" onClick={() => void agentOSApi.holdingDetail(accountId, item.instrument_id).then(setDetail).catch((error) => toast.error(error instanceof Error ? error.message : "Detail unavailable"))}>{item.name}</Button>)}</div></Panel> : null}
        </TabsContent>
    ...
    <HoldingSheet detail={detail} close={() => setDetail(null)} locale={locale} />
  </DashboardPage>;
}
...
function HoldingSheet({ detail, close, locale }: { detail: HoldingDetail | null; close: () => void; locale: string }) {
  return <Sheet open={Boolean(detail)} onOpenChange={(open) => { if (!open) close(); }}><SheetContent className="w-full overflow-y-auto border-agent-border bg-agent-chrome text-agent-text sm:max-w-[560px]"><SheetHeader><SheetTitle>{detail?.position.name || "—"}</SheetTitle><SheetDescription>{detail ? `${detail.position.symbol} · ${detail.position.instrument_type} · ${detail.position.price_source || "missing"} · ${detail.position.price_as_of || "—"}` : ""}</SheetDescription></SheetHeader>{detail ? <div className="mt-6 space-y-4"><Panel><SectionTitle title={locale === "zh" ? "价格历史" : "Price History"} en="OFFICIAL SOURCE" />{detail.history.available ? <div className="max-h-44 overflow-y-auto divide-y divide-agent-border">{detail.history.items.slice(-30).reverse().map((item) => <div key={item.date} className="flex justify-between py-2 font-data text-[10px]"><span className="text-agent-dim">{item.date}</span><span>{item.price}</span></div>)}</div> : <EmptyPanel title={locale === "zh" ? "价格历史不可用" : "Price history unavailable"} detail={detail.history.reason || "—"} />}</Panel><Panel><SectionTitle title={locale === "zh" ? "财务 / 净值证据" : "Fundamentals / NAV Evidence"} en="POINT IN TIME" />{detail.fundamentals.available ? <pre className="max-h-64 overflow-auto whitespace-pre-wrap font-data text-[10px] text-agent-muted">{JSON.stringify(detail.fundamentals.data, null, 2)}</pre> : <EmptyPanel title={locale === "zh" ? "正式证据不可用" : "Formal evidence unavailable"} detail={detail.fundamentals.reason || "—"} />}</Panel><Panel><SectionTitle title={locale === "zh" ? "风险与衍生品" : "Risk & Derivatives"} en="NO SYNTHETIC GREEKS" /><p className="text-xs leading-6 text-agent-muted">{detail.derivatives.available ? JSON.stringify(detail.derivatives) : detail.derivatives.reason || detail.portfolio_context.risk_reason || (locale === "zh" ? "缺少可审计收益序列、Greeks 或波动率曲面。" : "Auditable returns, Greeks or volatility surface are missing.")}</p></Panel></div> : null}</SheetContent></Sheet>;
}
...
function PositionDialog({ open, setOpen, form, setForm, submit, saving, locale }: { open: boolean; setOpen: (value: boolean) => void; form: PositionForm; setForm: (value: PositionForm) => void; submit: (event: FormEvent) => void; saving: boolean; locale: string }) {
  return <Dialog open={open} onOpenChange={setOpen}><DialogContent><DialogHeader><DialogTitle>{locale === "zh" ? "记录持仓" : "Add position"}</DialogTitle><DialogDescription>{locale === "zh" ? "必须选择正式标的类型与供应商代码；不能可靠识别时保留 manual。" : "Choose an official instrument type and provider symbol; uncertain legacy assets remain manual."}</DialogDescription></DialogHeader><form onSubmit={submit} className="flex flex-col gap-4"><div className="grid gap-3 sm:grid-cols-2"><div><Label>{locale === "zh" ? "标的类型" : "Instrument type"}</Label><Select value={form.instrument_type} onValueChange={(instrument_type) => setForm({ ...form, instrument_type, asset_class: instrument_type })}><SelectTrigger className="mt-1.5"><SelectValue /></SelectTrigger><SelectContent><SelectGroup>{["stock", "etf", "open_fund", "future", "option", "convertible_bond", "cash", "fx", "alternative", "manual"].map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectGroup></SelectContent></Select></div><div><Label>{locale === "zh" ? "方向" : "Direction"}</Label><Select value={form.direction} onValueChange={(direction) => setForm({ ...form, direction })}><SelectTrigger className="mt-1.5"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="long">long</SelectItem><SelectItem value="short">short</SelectItem></SelectContent></Select></div>{[["symbol", "代码 / Symbol"], ["provider_symbol", "Provider symbol"], ["name", locale === "zh" ? "名称" : "Name"], ["quantity", locale === "zh" ? "数量" : "Quantity"], ["multiplier", locale === "zh" ? "合约乘数" : "Multiplier"], ["price", locale === "zh" ? "成交 / 手工净值" : "Trade / manual NAV"], ["trade_date", locale === "zh" ? "截至日期" : "As of"]].map(([key, label]) => <div key={key} className="flex flex-col gap-1.5"><Label htmlFor={`position-${key}`}>{label}</Label><Input id={`position-${key}`} type={["quantity", "price", "multiplier"].includes(key) ? "number" : key === "trade_date" ? "date" : "text"} step="any" value={form[key]} required={!["provider_symbol"].includes(key)} onChange={(event) => setForm({ ...form, [key]: event.target.value })} /></div>)}</div><DialogFooter><Button type="submit" disabled={saving}>{locale === "zh" ? "保存到组合账本" : "Save to ledger"}</Button></DialogFooter></form></DialogContent></Dialog>;
}
