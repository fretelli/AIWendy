...
| Redis | 7.2 | Cache, task coordination, worker heartbeats |
...
Only the latest tagged release and the current `v2` branch receive fixes. Database migrations are forward-only; a minor release can intentionally retire public APIs while preserving historical tables for upgrade compatibility. Before upgrading, back up PostgreSQL and record the currently deployed image digests.
