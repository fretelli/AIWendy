...
默认 Docker Compose 配置会以 `NEXT_PUBLIC_AUTH_REQUIRED=1` 构建 Web，启动后需要登录。公网或生产部署必须保持 `KEELTRADER_AUTH_REQUIRED=1` 和 `NEXT_PUBLIC_AUTH_REQUIRED=1`。设置 `KEELTRADER_AUTO_INIT_TEST_USERS=1` 可在首次启动时创建测试账号。
